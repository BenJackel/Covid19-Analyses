...
def smooth_data(df, window):
    df["R(t) (smoothed)"] = (
        df.set_index("Date")["R(t)"].rolling(f"{window}d").mean().values
    )
    df["New Cases (smoothed)"] = (
        df.set_index("Date")["New Cases"].rolling(f"{window}d").mean().values
    )
    df["Active Cases (smoothed)"] = (
        df.set_index("Date")["Active Cases"].rolling(f"{window}s").mean().values
    )
    return df
...
def plot_R(data, window):
    ...
    color = alt.condition(
        alt.datum["R(t) (smoothed)"] > 1.0, alt.value("red"), alt.value("green")
    )

    R = (
        alt.Chart(df)
        .mark_line(stroke="#662E9B")
        .transform_fold(
    ...
    R_points = R.mark_point().encode(
        opacity=alt.condition(nearest, alt.value(1), alt.value(0)), color=color
    )
    ...
    R_text = R.mark_text(align="left", dx=5, dy=-5).encode(
        text=alt.condition(nearest, "R(t) (smoothed):Q", alt.value(" ")), color=color
    )

    # Draw some rectangles to shade
    areas = R.mark_rule(opacity=0.2).encode(color=color)

    C_text = C.mark_text(align="left", dx=5, dy=-5, stroke="blue").encode(
        text=alt.condition(nearest, "New Cases (smoothed):Q", alt.value(" "))
    )
    ...
    date_text = rules.mark_text(align="left", dx=5, dy=0, stroke="blue").encode(
        text="Date:T"
    )
    ...
    shape_color_scale = alt.Scale(
        domain=["Tighten", "Ease", "Event"], range=["red", "green", "blue"]
    )
    interventions = (
        alt.Chart(int_df)
        ...
        .encode(
            ...
            shape=alt.Shape("Type:N", scale=shape_scale, title="Intervention Type"),
    ...
    daily_case_chart = (
        alt.Chart(daily_cases_df)
        .mark_bar(opacity=0.25, color="grey")
        .encode(x=alt.X("Date:T"), y=alt.Y("New Cases:Q"))
    )
    ...
    chart = alt.layer(chart, layer3).resolve_scale(
...
def plot_acceleration():
    ...
    df.loc[50:55, "Acceleration"] = -0.1
    df.loc[150:153, "Acceleration"] = -0.1

    df = integrate_a(df)

    A = (
        alt.Chart(df)
        .mark_line(stroke="green")
        .encode(
            x=alt.X("Date:T", axis=alt.Axis(title="Date")),
...
def app():
    ...
    province_selection = st.sidebar.selectbox(
        "Select Province", options=prov_options, index=default_index
    )
